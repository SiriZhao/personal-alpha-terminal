...
from personal_alpha_terminal.desktop.runtime import application_data_dir, process_is_running
...
class ConsoleInstanceLock(AbstractContextManager["ConsoleInstanceLock"]):
    ...
    def __enter__(self) -> ConsoleInstanceLock:
        ...
        if pid and pid != os.getpid() and process_is_running(pid):
            raise RuntimeError(f"Personal Alpha Terminal 已在运行（PID {pid}）")
        temporary = self.path.with_suffix(".tmp")
        temporary.write_text(
            json.dumps({"pid": os.getpid(), "executable": sys.executable}), encoding="utf-8"
        )
